...
window::~window() {
    ...
    int iX, iY;
    const bool windowStillExists = (hwndWindow != NULL) && (IsWindow(hwndWindow) == TRUE);

    if(dcScreen != NULL) {
        if(windowStillExists && ReleaseDC(hwndWindow, dcScreen) == 0) {
...
inline void window::ProcessMessage() {
    ...
    if(hwndWindow == NULL || IsWindow(hwndWindow) != TRUE) {
        return;
    }
...
bool window::CanDraw() const {
    ...
    return dcActive != NULL && hwndWindow != NULL && IsWindow(hwndWindow) == TRUE;
...
bool window::IsOpen() const {
    ...
    return hwndWindow != NULL && IsWindow(hwndWindow) == TRUE;
...
void window::StoreImage(image *imgThis, const unsigned usX, const unsigned short usY, const unsigned short usWidth, const unsigned short usHeight) {
    ...
    if((usX + usWidth > static_cast<unsigned>(iWindowWidth)) || (usY + usHeight > static_cast<unsigned short>(iWindowHeight))) {
